...
public class SPPUtils {
	...
	public static boolean pair(String strAddr, String strPsw) {
		...
		if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
			try {
				...
				SPPUtils.setPin(device.getClass(), device, strPsw); // 手机和蓝牙采集器配对
				SPPUtils.createBond(device.getClass(), device);
	...
	discoveryFilter.addAction(ACTION_PAIRING_REQUEST); 
	...
	// 搜索未配对的设备
	context.registerReceiver(brReceiver, discoveryFilter);[/mw_shl_code]

	
	BroadcastReceiver brReceiver= new BroadcastReceiver() 
	{
	...
	ClsUtils.cancelPairingUserInput(btDevice.getClass(), btDevice); //某些手机在此处会抛出异常，若果有需要处理的可在catch中处理
	}
	catch (Exception e)
	{
	// TODO Auto-generated catch block
